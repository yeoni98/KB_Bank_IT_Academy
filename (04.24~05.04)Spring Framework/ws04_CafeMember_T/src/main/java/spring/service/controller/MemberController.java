...
public class MemberController {
	...
	public String login(HttpServletRequest request, MemberVO pvo) throws Exception{
		// MemberVO로 자동 바인딩
		// model 이 아니라 session에다가 바인딩 해야함. 로그인이랑 업데이트는 반드시 session에다가 바인딩, 나머지는 model에다가 바인딩.
	...
	public String showAll(Model model ) throws Exception{
		...
		// 리턴 타입이 스트링으로 간단해진다. 리스트가 아니라
		return "allView";
	}
	@RequestMapping("update.do")
	public String update(HttpServletRequest request, MemberVO pvo) throws Exception{
		memberDAO.updateMember(pvo);
		
		//로그인 된 상태에서만 가능함.
		if(request.getAttribute("vo")!=null) {//로그인 된 상태라면
	...
	public String logout(HttpServletRequest request) throws Exception{
		...
		if(session.getAttribute("vo")!=null) { //로그인 된 상태라면
			session.invalidate(); // 세션 죽이는게 로그아웃
			return "logout";
		}
		return null;
	}
